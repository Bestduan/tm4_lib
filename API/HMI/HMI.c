#include "HMI.h"


/****************************************���Ҳ�����************************************/
const unsigned char SIN_TABLE[]={
};

//******************************************
//�������ƣ�HMICommandPut
//�������ܣ��������ָ��	ʹ��UART4
//��    ����const uint8_t *Data	-- �ַ�������
//����ֵ��  ��
//******************************************
void HMICommandPut(const char *Data)
{
	uint8_t index;
	UARTprintf(4,"%s",Data);
//	while(*Data != 0x00)
//	{
//	    UARTCharPut(UART4_BASE,*Data++);
//	}
	for(index=0;index<3;index++)	//ָ�����
	{
	    UARTCharPut(UART4_BASE,0xFF);
	}
}



//******************************************
//�������ƣ�HMIDrawCurve
//�������ܣ���������	ʹ��UART4
//��    ����uint32_t Num   --�����а�����ĸ���
//		    const uint8_t *Data -- ���ݴ��������׵�ַ
//����ֵ��  ��
//******************************************
void HMIDrawCurve(const unsigned char *Data ,uint32_t Num)
{
	uint32_t index;
	uint16_t temp;
	
	for(index=0;index<Num;index++)
	{	
		char str[14] = {0};
		temp = (uint16_t)((*(Data+index))/2+60);
		sprintf(str,"add 10,0,%d",temp);	//����ID�ţ�10 ͨ����0
		HMICommandPut(str);
		delay_ms(1);
	}
		
}

//******************************************
//�������ƣ�HMIShowText
//�������ܣ���ʾָ�����ı�����
//��    ����			uint32_t TextNum	 ----�ı��ڴ������еĵ����
//					uint8_t *Data			 ----Ҫ��ʾ���ݵ��׵�ַ
//					uint32_t Len   	 	 ----�����ַ����ĳ���
//����ֵ��  ��
//******************************************
void HMIShowText(uint32_t TextNum, const char *Data)
{
	char str[30]={0};
	
	sprintf(str,"t%d.txt=\"%s\"",TextNum,Data);	
	HMICommandPut(str);
}

